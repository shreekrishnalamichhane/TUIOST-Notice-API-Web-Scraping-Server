...
const ResultController = {
    watch: async (req: Request, res: Response) => {
        try {
            let page = await ScraperService.scrape(req.browser, TUIOST_URL + '/result');
            if (page) {
    ...
    all: async (req: Request, res: Response) => {
        try {
            let pages = await Utils.getTotalPages(req.browser, TUIOST_URL + '/result');
            let datas: any = [];

            for (let index = 0; index < pages; index++) {
                let page = await ScraperService.scrape(req.browser, TUIOST_URL + '/result?page=' + index);
                if (page) {
                    let data = await ScraperService.sanitize(page);
                    datas = [...datas, ...data]
                }
